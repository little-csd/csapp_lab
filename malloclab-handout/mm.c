 ...
 *      malloc some block for the link headers.
 *      The minimums block is 16 bytes
 */
int mm_init(void)
{
    char* start;
...
/*
 * getSizeClass - calculate size belongs to which size class
 */
static size_t getSizeClass(size_t size)
{
    size--;
...
/*
 * extend_heap - extend the heap by msize DWORD blocks.
 */
static void* extend_heap(size_t msize)
{
...
/*
 * findFitAndRemove - find a block with size or larger.
 *      If found, remove the block, and return the address.
...
/*
 * delete_node - delete the node with an address ptr in sizeClass.
 */
static void delete_node(void* ptr, size_t sizeClass)
{
...
/*
 * add_node - add the node with an address ptr in sizeClass.
 */
static void add_node(void* ptr, size_t sizeClass)
{
    char* cur_node = start_link_list + sizeClass * WSIZE;
...
/*
 * coalesced - coalesced the block if necessary.
 *      return the address of coalesced block.
 */
static void* coalesced(void *ptr)
{
...
/*
 * mm_check - Check the heap whether there are some problems.
 */
static void mm_check()
{
    printf("\nNow is checking...\n");
    size_t sizeClass = 0;
    char* link_begin = start_link_list;
    while(link_begin != end_link_list) {
        char* cur_node = LINK_NEXT(link_begin);
        int nodeNum = 1;
        if (cur_node != NULL)
            printf("Current size class = %d.\n", sizeClass);
        sizeClass++;
        while(cur_node != NULL) {
            printf("Node %d : \n  Begin at : %p\n  Size : %u\n  Allocated : %u\n  Prev_allocated : %u\n", nodeNum++, cur_node, GET_SIZE(HDRP(cur_node)), GET_ALLOC(HDRP(cur_node)), GET_PREV_ALLOC(HDRP(cur_node)));
            printf("  Head: %u\n  Foot: %u\n  NEXT_PTR: %x\n  LAST_PTR: %x\n", GET(HDRP(cur_node)), GET(FTRP(cur_node)), GET(NEXT_PTR(cur_node)), GET(LAST_PTR(cur_node)));
            cur_node = LINK_NEXT(cur_node);
        }
        link_begin += WSIZE;
    }
